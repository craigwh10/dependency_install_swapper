...
function handleReplaceTextFromClipboard (cmdPrefix: string, packageNameOrDevDep: string, packageNameOrNull: string | null): void {
  // yarn add -D package
...
function handleReplaceTextFromPath (cmdPrefix: string): void {
  const { installButton: { el } } = getInstallButton()
  const warning = document.querySelector('#dis-google-ext-warning')
